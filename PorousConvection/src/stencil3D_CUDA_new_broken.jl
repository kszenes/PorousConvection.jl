...
@parallel_indices (ix, iy, iz) function compute_flux_p_3D!(
    ...
    if (ix <= nx && iy <= ny && iz <= nz)
        ...
        if (ix > 1 && @threadIdx().x == 1) P_l[tx-1,ty,tz] = Pf[ix-1,iy,iz] end
        if (iy > 1 && @threadIdx().y == 1) P_l[tx,ty-1,tz] = Pf[ix,iy-1,iz] end
        if (iz > 1 && @threadIdx().z == 1) P_l[tx,ty,tz-1] = Pf[ix,iy,iz-1] end
        @sync_threads()
        if (ix != 1)
            qDx_tmp = qDx[ix,iy,iz] # Load into register
            qDx[ix,iy,iz] = qDx_tmp - _1_θ_dτ *
                                (qDx_tmp + k_ηf * _dx *
                                (P_l[tx,ty,tz]-P_l[tx-1,ty,tz])
                            )
        end
        if (iy != 1)
            qDy_tmp = qDy[ix,iy,iz] # Load into register
            qDy[ix,iy,iz] = qDy_tmp - _1_θ_dτ * (
                                qDy_tmp + k_ηf * _dy *
                                (P_l[tx,ty,tz]-P_l[tx,ty-1,tz])
                            )
        end
        if (iz != 1)
            qDz_tmp = qDz[ix,iy,iz] # Load into register
            qDz[ix,iy,iz] = qDz_tmp - _1_θ_dτ * (
                                qDz_tmp + k_ηf * _dz *
                                (P_l[tx,ty,tz]-P_l[tx,ty,tz-1]) -
                                αρg * 0.5 * (T[ix,iy,iz] + T[ix,iy,iz-1])
                            )
        end
    end
    return nothing
...
@parallel_indices (ix, iy, iz) function compute_flux_T_3D!(
    ...
    if (ix < nx - 1 && iy < ny - 1 && iz < nz - 1)
        ...
        # Load data into shared memory
        if (x > 1 && @threadIdx().x == 1) T_l[tx-1,ty,tz] = T[ix-1,iy,iz] end
        if (iy > 1 && @threadIdx().y == 1) T_l[tx,ty-1,tz] = T[ix,iy-1,iz] end
        if (iz > 1 && @threadIdx().z == 1) T_l[tx,ty,tz-1] = T[ix,iy,iz-1] end
        @sync_threads()

        if (ix <= nx-1 && iy <= ny-2 && iz <= nz-2)
            # qTx_tmp = qTx[ix,iy,iz] # Load into register
        ...
        if (ix <= nx-2 && iy <= ny-1 && iz <= nz-2)
            # qTy_tmp = qTy[ix,iy,iz] # Load into register
        ...
        if (ix <= nx-2 && iy <= ny-2 && iz <= nz-1)
            # qTz_tmp = qTz[ix,iy,iz] # Load into register
